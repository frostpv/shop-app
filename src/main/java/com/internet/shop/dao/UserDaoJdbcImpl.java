...
public class UserDaoJdbcImpl implements UserDao {
    ...
    private void getUserRoles(User user){
        Set <Role> roles = new HashSet<>();
        String query = "SELECT * FROM user_roles JOIN roles ON roles.role_id = user_roles.role_id WHERE user_id = ?";
        try (PreparedStatement preparedStatement = ConnectionUtil.getConnection().prepareStatement(query)) {
            preparedStatement.setLong(1, user.getId());
            ResultSet resultSet = preparedStatement.executeQuery();
    ...
    public User update(User user) {
        String query = "UPDATE users SET user_name = ?, user_login = ?, user_pass = ? WHERE user_id = ?";
        try (PreparedStatement preparedStatement = ConnectionUtil.getConnection().prepareStatement(query)) {
            preparedStatement.setString(1, user.getName());
            preparedStatement.setString(2, user.getLogin());
            preparedStatement.setString(3, user.getPassword());
